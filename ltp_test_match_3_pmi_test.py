...
path_corpus = open(path+"/flag_sentence_test.txt",'r')
...
path_pmi_all = open(path+"/pro_sen_pmi_weibo_sort1.txt")

path_out = open(path+'/jieguo_short_20_weibo_pro_necessary_flag_pmi_3_test1.txt','w')
#path_corpus = open("/data0/shenyanjun/lexicon/car_review_split.txt")
#path_out = open(path+'/test_long.txt','w')
...
#读取pmi结果
def read_pmi(path):
    return {(line.split('\t')[0],line.split('\t')[1]):float(line.split('\t')[2]) for line in path.readlines()}
...
def select_patten(property_list,sentiment_list,degree_list,parse_sentence):
    ...
    return sentence_pro,sentence_sen,sentence_deg,locations,location_sen,location_pro,location_deg

def check_out(path_X,model_X,parse_sentence,locations,path):
    sentence_path_x = []
        ...
        if path_one in model_X:
            ...
            for location,element_x in enumerate(path_one_copy):
                if isinstance(element_x,int) and element_x in locations:
                    ...
                    aa += parse_sentence[element_x]['cont']
                elif isinstance(element_x,int) and element_x not in locations:
                    aa += parse_sentence[element_x]['cont']
                else:
                    aa += element_x
        if len(aa)>0:
...
if __name__ == '__main__':
    ...
    print degree_list[0],len(degree_list)
    corpus = read_corpus(path_corpus)
    pmi_all = read_pmi(path_pmi_all)
    print pmi_all.keys()[0]
    patten_1 = []
    patten_2 = []
    patten_3 = []
    jieguo_count = 0
    for line in corpus:
        ...
        for parse_sentence in parse_line:
            path_sentence_out = []
            juzi_pro,juzi_sen,juzi_deg,locations,location_sen,location_pro,location_deg = select_patten(property_list,sentiment_list,degree_list,parse_sentence)
            ...
            print "path_all_in:",path_all_in
            print "path_sen_pro:",path_sen_pro
            path_1_out = check_out(path_all_in,model1,parse_sentence,locations,path_out)
            path_2_out = check_out(path_sen_pro,model2,parse_sentence,locations,path_out)
            if path_1_out and path_1_out not in path_sentence_out:
                print "path_all-in:",'ok1'
        ...
        if path_line_out:
            ...
            #     print parse_sentence[pro]['cont']
                #path_out.write(parse_/sentence[pro]['cont']+'\t')
            print '----result------'
            path_out.write('\n')
            ...
            path_out.write('---pmi---'+'\n')
            pmi_out = []
            for word_list in path_line_out:
                words_pro = []
                words_sen = []
                words_deg = []
                for word in word_list:
                    if word in property_list:
                        words_pro.append(word)
                    elif word in sentiment_list:
                        words_sen.append(word)
                    else:
                        words_deg.append(word)
                if len(words_deg) > 1:
                    words_deg = ['\t'.join(words_deg)]
                print words_deg,len(words_deg)
                for word_pro in words_pro:
                    print word_pro
                    for word_sen in words_sen:
                        print word_sen
                        if (word_pro,word_sen) in pmi_all and pmi_all[(word_pro,word_sen)] >0 and len(words_deg) > 0:
                            pmi_out.append(word_pro+'\t'+word_sen+'\t'+words_deg[0]+'\n')
                        elif (word_pro,word_sen) in pmi_all and pmi_all[(word_pro,word_sen)] >0 and len(words_deg) < 1:
                            pmi_out.append(word_pro+'\t'+word_sen+'\n')
                        else:
                            pass
            for pmi_cizu in list(set(pmi_out)):
                path_out.write('\t\t'+pmi_cizu)
            path_out.write('\n')
            path_out.write('----------------------------------------------------------------------------------------------------'+'\n')
